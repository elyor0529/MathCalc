...
import uz.example.calc.parser.collection.ArrayStack
import uz.example.calc.parser.enum.FunctionalOperators
import uz.example.calc.parser.enum.NormalOperators
import uz.example.calc.parser.exception.BadSyntaxException
import uz.example.calc.parser.exception.NotFoundException
import uz.example.calc.parser.exception.DomainException
import uz.example.calc.parser.exception.NotSupportedException
import uz.example.calc.parser.helper.isIn
import uz.example.calc.parser.helper.notIn
import kotlin.math.*

class ExpressionParser {

    private val numStack = ArrayStack<Double>()
    private val opStack = ArrayStack<String>()
    ...
    private fun evaluateExpression(expression: String): Double {
        ...
        while (i < expression.length) {
            ...
            if (currChar in "0123456789.") {
                ...
                if (i != 0 && (expression[i - 1] == ')' || expression[i - 1] == 'e' || (i >= 2 && expression.substring(i - 2, i) == "PI"))) {
